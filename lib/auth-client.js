    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const handleMessage = async (event) => {
          console.log('Google popup postMessage event:', event);
        ...
        // Only reject if popup closes and we never got a message
        const checkClosed = setInterval(() => {
          if (popup.closed) {
            clearInterval(checkClosed);
            window.removeEventListener('message', handleMessage);
            if (!resolved) {
              reject(new Error('Authentication was canceled or did not complete.'));
            }
          }
        }, 500);
      });
